...
fn main() {
  if std::env::var("GITHUB_ACTIONS").is_err() {
    // Load env variables from ".env" file, if not exists, use ".env" to set default value.
  ...
  // Iterate over all env variables and print those starting with "AML_" for compilation (env variables can not be accessed directly in compile time)
